...
async function main() {
  ...
  async function moveCursor(targetX, targetY, durationMs, fps = 15) {
    ...
    for (let i = 1; i <= frames; i++) {
      ...
      await page.evaluate((x, y) => window.updateCursor(x, y, false), cursorX, cursorY);
      await captureFrame();
      await sleep(interval);
    }
  }
  ...
  for (const b of buttons) {
    ...
    if (text && text.includes('Transcrire les factures')) {
      transcribeButton = b;
      break;
    }
  }
  ...
  console.log('Recording: Step 5 - Waiting for transcription loading and completion...');
  ...
  console.log('Recording: Step 6 - Inspecting transcribed JSON and Download button...');
  ...
  if (downloadBtn) {
    ...
    await moveCursor(Math.round(dlBox.x + dlBox.width / 2), Math.round(dlBox.y + dlBox.height / 2), 600);
  }
  ...
  console.log(`\nDONE!`);
  console.log(`GIF: ${gifOutput} (${(gifStats.size / (1024 * 1024)).toFixed(2)} MB)`);
  console.log(`MP4: ${mp4Output} (${(mp4Stats.size / (1024 * 1024)).toFixed(2)} MB)`);
}
